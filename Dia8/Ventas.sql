...
-- select * from pedido;
delimiter //
create function total_pedidos(id_cliente_in int, total double)
returns double deterministic
begin
	declare total_cantidad double;
    
    select total into total_cantidad from pedido
   where id_cliente = id_cliente_in;
    return total_cantidad;
end //
delimiter ;

select  total_pedidos(1, total) from pedido;